...
const searchNode = async () => {
    let val = document.getElementById('SearchNodeField').value;
    if (val === '') return;
    if (isNaN(val)) {
      alert('Only integer values are allowed');
      return;
    ...
    val = parseInt(val);
    document.getElementById('SearchNodeField').value = '';
    freezeButtons();
    ...
    while (true) {
      ...
      if (node.value === val) {
        // Found the node which we want to search.
        nodeEle.className.baseVal = 'highlightedNode greenNode';
        break;
      } else if (node.value > val) {
        // Go to left subtree if value is smaller.
        if (node.children[0].value !== null) {
          const linkElement = document.getElementById(`link_Source_${node.nodeId}_Dest_${node.children[0].nodeId}`);
          if (linkElement) linkElement.className.baseVal = 'LinkAnimation';
          nodeEle.className.baseVal = 'highlightedNode redNode';
          node = node.children[0];
        } else {
        ...
        if (node.children[1].value !== null) {
          const linkElement = document.getElementById(`link_Source_${node.nodeId}_Dest_${node.children[1].nodeId}`);
          if (linkElement) linkElement.className.baseVal = 'LinkAnimation';
          nodeEle.className.baseVal = 'highlightedNode redNode';
          node = node.children[1];
        } else {
  ...
  // If user presses enter key, trigger search function.
  document.getElementById('SearchNodeField').addEventListener('keyup', function (event) {
    if (event.key === 'Enter') {
